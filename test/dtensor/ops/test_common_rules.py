...
from vescale.dtensor.op_schema import OpSchema
from vescale.dtensor.ops.common_rules import einop_rule, pointwise_rule
from vescale.dtensor.placement_types import DTensorSpec, TensorMeta
...
class CommonRulesTest(DTensorTestBase):
    ...
    @with_comms
    def test_einop_bmm_propagation(self):
        # plain einsum, mm
        mesh = DeviceMesh(self.device_type, torch.arange(self.world_size))

        mm_call = aten.mm.default
        # propagate col-wise sharding
        mat1, mat2 = [-1, 0, -1], [-1, -1, -1]

        mat1_tensor_meta = self._gen_tensor_meta(torch.Size([4, 8, 4]))
        mat2_tensor_meta = self._gen_tensor_meta(torch.Size([4, 4, 8]))
        ...
        output_sharding = einop_rule("bmk,bkn->bmn", OpSchema(mm_call, (mat1_spec, mat2_spec), {}))
