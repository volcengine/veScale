...
import logging
from typing import Any, Callable, Dict, Generator, Iterator, List, Sequence, Tuple, TypeVar, cast, Union
...
def redistribute_profiler() -> Generator[RedistributeProfile, None, None]:
    ...
    orig_redistribute_local_tensor = vescale.parallel.mp.comm.api.redistribute_local_tensor
    ...
    try:
        # pyre-ignore[9]
        vescale.parallel.mp.comm.api.redistribute_local_tensor = patched_redistribute_local_tensor
        yield profile
    finally:
        vescale.parallel.mp.comm.api.redistribute_local_tensor = orig_redistribute_local_tensor
...
class DTensorTestBase(MultiProcessTestCase):
    ...
    def _start_processes(self, proc) -> None:
        self.processes = []
        processor_num = 1 if self.backend == "meta" else int(self.world_size)
        for rank in range(processor_num):
    ...
    def assertAllClose(
        ...
        x,
        y,
        msg: Union[str, Callable[[str], str]] | None = None,
        *,
        atol=None,
        rtol=None,
        equal_nan=True,
    ...
    def assertNotEqual(
        ...
        x,
        y,
        msg: str | None = None,  # type: ignore[override]
        *,
        atol=0,
        rtol=0,
        **kwargs,
    ) -> None:
        assert atol == 0 and rtol == 0, "veScale's DTensorTestBase only allows atol==0 and rtol==0"
...
def with_comms(
    _func: TestFunc | None = None,
    *,
    backend_for_cuda: str = "nccl",
    backend_for_cpu: str = "gloo",
) -> Callable[[TestFunc], TestFunc]:
    def _decorator(func: TestFunc) -> TestFunc:
        ...
        def wrapper(self, *args: Tuple[object], **kwargs: Dict[str, Any]) -> None:  # type: ignore[misc]
            ...
            if torch.cuda.is_available() and torch.cuda.device_count() >= self.world_size:
                ...
                self.backend = backend_for_cuda
            else:
                self.device_type = "cpu"
                self.backend = backend_for_cpu
...
def init_random_seeds(seed):
    """general purpose random seeding"""
    random.seed(seed)
    np.random.seed(seed)
