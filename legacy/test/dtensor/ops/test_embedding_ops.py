...
from vescale import DTensor, distribute_tensor
...
class TestEmbeddingOp(DTensorTestBase):
    def _run_embedding_op_test(
        ...
        **kwargs,
        ...
        torch.manual_seed(0)
        local_embedding = torch.nn.Embedding(  # global shape
            num_embeddings,
            embedding_dim,
            device=self.device_type,
            **kwargs,
        )
        sharded_embedding = torch.nn.Embedding(  # sharded shape
            num_embeddings,
            embedding_dim,
            device=self.device_type,
            **kwargs,
        ...
        output = sharded_output.redistribute(sharded_output.device_mesh, [Replicate()]).to_local()
        ...
        gradient = sharded_embedding.weight.grad.redistribute(sharded_output.device_mesh, [Replicate()]).to_local()
        ...
        self.assertEqual(
            local_output,
            sharded_output.redistribute(sharded_output.device_mesh, [Replicate()]).to_local(),
        )
