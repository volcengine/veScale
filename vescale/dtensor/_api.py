...
from vescale.dtensor._collective_utils import check_tensor_meta, mesh_broadcast
from vescale.dtensor._dtensor_spec import DTensorSpec, TensorMeta
from vescale.dtensor._dispatch import OpDispatcher
from vescale.dtensor._utils import (
    compute_local_shape_and_global_offset,
    compute_global_tensor_info,
    normalize_to_torch_size,
)
from vescale.dtensor._redistribute import (
    Redistribute,
...
"""
1. _shard_tensor is removed as already deprecated by pytorch
2. distribute_module is removed as we do not plan to support it.
3. the followings are kept but unmodified as they return dtensor:
    - _ToTorchTensor
    - _FromTorchTensor
    - distribute_tensor
    ...
    - randn
    - zeros
4. DTensor is modified with:
    - Vescale DTensor inherit torch dtensor.
    - _op_dispatcher is vescale _op_dispatcher.
    - __new__ is kept to create vescale dtensor
    - __torch_dispatch__ is kept to use our OpDispatcher
    - from_local is kept to use our _FromTorchTensor
    - to_local is kept to use our _ToTorchTensor
    - redistribute is kept to use our Redistribute
    - full_tensor is kept to  use our _ToTorchTensor
    - __tensor_unflatten__ is kept but unmodified as it returns a dtensor.
"""
...
class _FromTorchTensor(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: "DTensor"):  # type: ignore[override]
        ...
        if grad_output.placements != previous_placement:
            ...
            output = redistribute_local_tensor(local_tensor, current_spec, target_spec, is_backward=True)
...
class DTensor(TorchDTensor):
    ...
    * :class:`Partial`: Tensor is pending reduction on the devices of the ``DeviceMesh`` dimension
    ...
    _op_dispatcher: op_dispatch.OpDispatcher = OpDispatcher()
    ...
    def to_local(self, *, grad_placements: Sequence[Placement] | None = None) -> torch.Tensor:
        ...
        return _ToTorchTensor.apply(self, grad_placements)  # pyre-ignore[16]: autograd func
    ...
    def full_tensor(self, *, grad_placements: Sequence[Placement] | None = None) -> torch.Tensor:
        ...
        return _ToTorchTensor.apply(redist_res, grad_placements).view(self.shape)

    def __create_write_items__(self, fqn: str, object: Any):
        from vescale.dtensor.vescale_utils.checkpoint import (
        ...
        if hasattr(self._local_tensor, "__create_write_items__"):
            ...
            return _create_write_items_for_ragged_shard(fqn, object)
        else:
    ...
    def __create_chunk_list__(self):
        ...
        if hasattr(self._local_tensor, "__create_chunk_list__"):
            ...
            return _create_chunk_for_ragged_shard(self)
        else:
            raise RuntimeError("Unsupported tensor type!")
    ...
    def __get_tensor_shard__(self, index):
        ...
        if hasattr(self._local_tensor, "__get_tensor_shard__"):
            ...
            return _get_tensor_shard_for_ragged_shard(self, index)
        else:
            raise RuntimeError("Unsupported tensor type!")
    ...
    *,
    ...
    for idx, placement in enumerate(placements):
        if placement.is_shard():
            ...
            local_tensor = placement._shard_tensor(local_tensor, device_mesh, idx, src_data_rank)
        elif placement.is_replicate():
            placement = cast(Replicate, placement)
            local_tensor = placement._replicate_tensor(local_tensor, device_mesh, idx, src_data_rank)
        elif is_ragged_shard(placement):
            placement = cast(RaggedShard, placement)
            local_tensor = placement._ragged_shard_tensor(local_tensor, device_mesh, idx, src_data_rank)
        else:
            raise RuntimeError(
