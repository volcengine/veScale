...
"""
all pointwise ops are kept as they need to support RaggedShard.
"""
...
def pointwise_strategy(op_schema: OpSchema, linearity: bool = False) -> OpStrategy:
    ...
    return common_pointwise_strategy(op_schema.args_schema, followed_strategy, linearity)
...
def common_pointwise_strategy(
    ...
    # handle broadcasting
    common_shape = torch.broadcast_shapes(*[arg.shape for arg in args_schema if isinstance(arg, OpStrategy)])
    ...
    for placement_strategy in followed_strategy.strategies:
        ...
        for placement in spec_to_follow.placements:
            if isinstance(placement, RaggedShard):
                assert common_shape.numel() % sum(placement.local_units) == 0, (
                    f"RaggedShard {placement} should be a divisor of the elements in common shape {common_shape}"
                )
...
def list_pointwise_strategy(op_schema: OpSchema, linearity: bool = False) -> StrategyType:
    ...
    for child_idx, child_strtgy in enumerate(follow_strategy.childs):
        ...
        args_schema: list[OpStrategy] = [
            cast(OpStrategy, arg_strategy.childs[child_idx]) for arg_strategy in args_strategies
        ]
...
for op in for_each_ops:
    register_op_strategy(op, schema_info=RuntimeSchemaInfo(needs_pytree=True))(list_pointwise_strategy)

for op in for_each_linearity_ops:
    register_op_strategy(op, schema_info=RuntimeSchemaInfo(needs_pytree=True))(list_linear_pointwise_strategy)
