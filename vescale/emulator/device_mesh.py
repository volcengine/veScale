...
import torch
from vescale.dtensor.device_mesh import DeviceMesh as vescaleDeviceMesh
from vescale.emulator.distributed import (
    ProcessGroup,
...
class DeviceMesh:
    ...
    By default (`pg` is `None`), we use the default emulator ProcessGroup in this DeviceMesh class
    ...
    By giving an existing emulator ProcessGroup `pg`, we construct a device mesh from this `pg`,
    ...
    Args:
        ...
        pg (Optional[ProcessGroup]): the given emulator ProcessGroup. See above for expected behaviors.
    ...
    def _init_process_groups(self, pg: ProcessGroup):
        ...
        dim_group_infos: List[Tuple[Tuple[str, List[int]]]] = []
        ...
        if self.mesh.ndim == 1 and self.mesh.numel() == _get_group_size(pg):
            ...
            for dim in range(self.mesh.ndim):
                ...
                dim_group_infos_by_dim: List[Tuple[str, List[int]]] = []
                for dim_mesh in pg_ranks_by_dim:
                    subgroup_ranks = dim_mesh.tolist()
                    # call new_group regardless of the current rank in the
                    # pg or not, it's required that all ranks participate
                    # in subgroup construction
                    dim_group = new_group(ranks=subgroup_ranks, backend=self.backend)
                    dim_group_infos_by_dim.append((_get_group_tag(dim_group), subgroup_ranks))
                dim_group_infos.append(tuple(dim_group_infos_by_dim))
    ...
    def find_pg_by_tuple_of_ranks_and_tag(
        self, dim_group_info_by_dim: Tuple[Tuple[str, List[int]]]
    ) -> List[ProcessGroup]:
        """
        Find the process group by the given tuple of ranks and tag.

        Args:
            dim_group_info_by_dim (Tuple[str, List[int]]): the tuple of ranks and tag
            for the given mesh dimension.
        Returns:
            A :class:`ProcessGroup` object
        """
        pg_list = []
        for pg_tag, pg_ranks in dim_group_info_by_dim:
            pg_list.append(_find_pg_by_ranks_and_tag(pg_tag, pg_ranks))
        return pg_list
    ...
    def get_dim_groups(self, mesh_dim: Optional[int] = None) -> Union[ProcessGroup, List[ProcessGroup]]:
        ...
        if mesh_dim is not None:
            return self.find_pg_by_tuple_of_ranks_and_tag(self._dim_group_infos[mesh_dim])
        else:
            dim_groups = []
            for mesh_dim in range(self.mesh.ndim):
                dim_groups.append(self.find_pg_by_tuple_of_ranks_and_tag(self._dim_group_infos[mesh_dim]))
            return dim_groups
...
def init_device_mesh(
    ...
    *,
    ...
    """
    Initializes an emulator `DeviceMesh` based on `device_type`, `mesh_shape`, and `mesh_dim_names` parameters.
...
def dump_nccl_graph_for_mesh(emulator_mesh: DeviceMesh, vescale_mesh: vescaleDeviceMesh):
    """
    Dump NCCL graph for a given mesh.

    Args:
        emulator_mesh (DeviceMesh): The emulator mesh.
        vescale_mesh (vescale.dtensor.device_mesh.DeviceMesh): The Vescale mesh.
    """
    my_rank = vescale_mesh.get_rank()

    # dump all other process groups
    # get all pgs in vescale_mesh
    all_pgs_infos = vescale_mesh._dim_group_infos
    # get all pgs in emulator_mesh
    all_pgs_emulator_infos = emulator_mesh._dim_group_infos
    # flatten 2D list all_pgs_infos into 1D list
    all_pgs_emulator_infos = [item for sublist in all_pgs_emulator_infos for item in sublist]
    my_rank = vescale_mesh.get_rank()
    for tag, ranks in all_pgs_infos:
        emulator_pg = _find_pg_by_ranks_and_tag(tag, ranks)
        torch_pg = torch.distributed.distributed_c10d._find_pg_by_ranks_and_tag(tag, ranks)
        # dump nccl graph for emulator_pg
        dump_nccl_graph_for_pg(emulator_pg, torch_pg, my_rank)


def delete_nccl_graph_for_mesh(emulator_mesh: DeviceMesh):
    """
    Delete NCCL graph for a given mesh.
    ...
    # flatten 2D list all_pgs_infos into 1D list
    all_pgs_emulator_infos = [item for sublist in all_pgs_emulator_infos for item in sublist]
    for tag, ranks in all_pgs_emulator_infos:
        emulator_pg = _find_pg_by_ranks_and_tag(tag, ranks)
        delete_nccl_graph_for_pg(emulator_pg)
