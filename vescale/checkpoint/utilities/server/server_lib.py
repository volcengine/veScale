...
class ReportServicer(report_service_pb2_grpc.OmniStoreReportServiceServicer):
    ...
    async def Gather(self, req: report_service_pb2.OmniStoreGatherRequest, ctx: grpc.aio.ServicerContext):
        i = await self._record(self._gather_dict, req, ctx)
        resp = report_service_pb2.OmniStoreGatherResponse()
        if req.with_result:
            resp.contents.extend([v for k, v in sorted(i.contents.items(), key=lambda x: x[0])])

        return resp

    async def Broadcast(self, req: report_service_pb2.OmniStoreBroadcastRequest, ctx: grpc.aio.ServicerContext):
        i = await self._record(self._bc_dict, req, ctx)
        return report_service_pb2.OmniStoreBroadcastResponse(content=i.contents[req.src_rank])
    ...
    async def GetStatus(self, req: report_service_pb2.OmniStoreGetStatusRequest, ctx: grpc.aio.ServicerContext):
        async with self._l:
            b = pickle.dumps(
                {
        ...
        return report_service_pb2.OmniStoreGetStatusResponse(status=b)
...
async def async_serve(servicer, async_addr: _AsyncObj):
    ...
    report_service_pb2_grpc.add_OmniStoreReportServiceServicer_to_server(servicer, server)
...
def get_stub(addr: str):
    ...
    return report_service_pb2_grpc.OmniStoreReportServiceStub(channel)
...
def gather(
    stub: report_service_pb2_grpc.OmniStoreReportServiceStub,
    gather_rank: int,
    rank: int,
    obj,
    tag: str = None,
    ...
    tag = tag or _get_tag()
    req = report_service_pb2.OmniStoreGatherRequest(
...
def broadcast(
    ...
    resp = stub.Broadcast(
        report_service_pb2.OmniStoreBroadcastRequest(tag=tag, rank=rank, content=c_content, src_rank=src_rank),
        timeout=timeout,
    )
...
def get_server_status(stub: report_service_pb2_grpc.OmniStoreReportServiceStub):
    resp = stub.GetStatus(report_service_pb2.OmniStoreGetStatusRequest())
